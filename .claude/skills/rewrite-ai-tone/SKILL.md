...
- 大げさ・もったいぶった動詞を避け、**日常語の動詞で言う**（例：「持続させる」→「覚えさせる」「残す」）。
- 実務で使われない**造語的なキーワードや概念ラベルを作らない**（例：「プロジェクト記憶」）。普段の言い方に置き換える。
- きれいな**対句や体言止めの対比でまとめない**（例：「守らせる決まり、動かす手順」）。整った言い回しより、中身が素直に伝わる文にする。
